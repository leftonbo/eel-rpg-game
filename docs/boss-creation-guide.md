...
- **レベル0**: 沼のドラゴン、闇のおばけ、機械のクモ（初期ボス）
- **レベル1**: 運び屋のサソリ（砂漠系）
- **レベル2**: 海のクラーケン、アクアサーペント（海系）
- **レベル3**: 夢魔ちゃん（ゲストキャラクター系）
- **レベル4**: 蜜柑ドラゴン（ジャングル系）
- **レベル6**: クリーンマスター（遺跡・廃墟系）
- **レベル5,7,8,9,10**: 将来の拡張用

### エクスプローラー経験値獲得システム

1. **初回勝利記念品**: `(解禁レベル + 1)² × 100` 経験値
2. **初回敗北記念品**: `(解禁レベル + 1)² × 100` 経験値
...
- テーマに応じた適切な解禁レベルを設定
- 既存ボスとの難易度バランスを考慮
- 記念品名は「{ボス名}のたてがみ」（勝利）、「{ボス名}の粘液」（敗北）で自動生成
